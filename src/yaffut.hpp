...
private:
  ...
  static std::string ShortName(std::string const &name)
  {
...
public:
  ...
  void List(const std::string& name)
  {
	...
	std::cerr << i++ << ")\t";
    std::cerr << ShortName(it->first) << std::endl;
    }
  }
  void Run(const std::string& name)
  {
    ...
    for(Tests_t::const_iterator it = m_Tests.begin(); it != m_Tests.end(); ++it)
    {
      std::string shortName = ShortName(it->first);
      bool isSpike = (shortName.find( "Spike") == 0); // barson
      if( !isSpike )
        ++i;
      if( ("All" == name && !isSpike) || shortName == name || (EqualsSuiteName (name, it->first) && !isSpike) )
      {
        try
        {
          std::cerr << std::endl << i << ") " << shortName << std::flush;
          {
            it->second();
          }
  ...
  void Report ()
  {
    ...
    std::cerr << std::endl;
    std::cerr << "[TOTAL](" << m_pass + m_fail << '/' << size << ")" << std::endl;
    std::cerr << "[OK](" << m_pass << '/' << size << ")" << std::endl;
    if (m_fail)
      std::cerr << "[FAIL](" << m_fail << '/' << size << ")" << std::endl;
  }
...
inline void almost(double e, double a, double epsilon, const char* at = "", const char* expr = "")
{
  if(std::abs(e - a) > epsilon)
  {
    throw failure(e, a, at, expr);
  }
}
...
inline int
mainRedirect (int argc, const char* argv[])
{
    freopen("yaffutOut.txt", "w", stdout);
    
    int ret = Factory::Instance ().Main (argc, argv);
...
#define YAFFUT_ALMOST(e,a,eps) \
  yaffut::almost (e ,a, eps , __YAFFUT_AT__, "ALMOST(" #e " == " #a ") failed ")
#ifndef ALMOST
#define ALMOST YAFFUT_ALMOST
#endif
